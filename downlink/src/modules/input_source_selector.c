...
typedef struct
{
    DownlinkInputSource active_source;
...
{
    return g_input_source_context.active_source;
}

const char *downlink_input_source_get_current_name(void)
{
    return (g_input_source_context.active_source == DOWNLINK_INPUT_SOURCE_USB)
               ? "USB IN (USART3)"
               : "XBee IN (USART6)";
}
